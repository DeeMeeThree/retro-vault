...
(function () {
    ...
    function setImageWidths(img, startW) {
        ...
        const renderHeight = window.innerHeight * 0.88;
        const endW = img.naturalWidth * (renderHeight / img.naturalHeight);
    ...
    function closeModal() {
        if (!modal || !modal.classList.contains('open') || animating) return;
        animating = true;
        const img = track.querySelector('.shelf-image');
    ...
    window.addEventListener('beforeunload', () => {
        ...
        if (closeBackdrop) closeBackdrop.removeEventListener('click', closeModal);
